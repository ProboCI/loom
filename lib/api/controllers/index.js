...
var streams = {
  ...
  deleteByBuildId: function(req, res, next) {
    var buildId = req.params.buildId;
    req.log = req.log.child({buildId: buildId}, true);
    req.log.info('Deleting streams for build');

    var storage = new Storage(req.loomConfig.storage);

    if (typeof storage.deleteStreamsByBuildId !== 'function') {
      res.json({error: 'deleteStreamsByBuildId not supported by current storage backend'});
      return next();
    }

    storage.deleteStreamsByBuildId(buildId, function(err, deleted) {
      if (err) {
        return handleError(res, err, next);
      }

      req.log.info({deleted: deleted}, 'Deleted streams for build');
      res.json({status: 'deleted', buildId: buildId, deleted: deleted});
      return next();
    });
  },
