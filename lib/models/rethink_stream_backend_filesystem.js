...
class FileSystemStorage extends RethinkStorage {
  ...
  deleteStreamsByBuildId(buildId, cb) {
    var pattern = this.makeStreamFilePath(`build-${buildId}-*`);
    glob(pattern, (err, files) => {
      ...
      files.forEach((filePath) => {
        ...
        var newPath = path.join(dir, newSidPrefix + basename);
