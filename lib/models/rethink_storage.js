...
class RethinkStorage {
  ...
  createReadStream(streamId, opts){
    ...
    var streams = [createCurrentDataStream()]
    if(!notail){
      streams.push(createChangesStream())
    }
  ...
  saveStream(streamId, meta, opts, cb){
    ...
    meta.id = streamId
    return r.table(this.config.meta_table).insert(meta, {conflict: conflict}).run(cb)
  }
  ...
  deleteStream(streamId, cb){
    ...
    return r.table(this.config.logs_table)
      .filter({sid: streamId})
      .update({
        sid: r.row("sid").add(new_sid_postfix),
        origSid: r.row("sid")
      })
      .run(cb)
  }
